...
def get_nutrition_info(prompt):
    system_message = (
        "Если запрос пользователя является вопросом, то ответь словом 'вопрос'. "
        "Ответь словом 'ошибка', если в сообщении от пользователя не упоминается никакая еда, напитки или питательные вещества. "
        "Прочитай список продуктов ниже и посчитай состав белков, жиров и углеводов. "
        "Посчитай точно, насколько это возможно, при необходимости используй средние размеры продуктов или напитков, игнорируй тот факт, что данные могут различаться. "
...
def get_food_smalltalk_answer(prompt):
    system_message = (
        "Если вопрос пользователя относится к теме питания, еды, спорта или диеты, то ответь так, "
        "как бы ответил опытный бот-диетолог. Если вопрос по другой теме, то сообщи, что пока не можешь "
